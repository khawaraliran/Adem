...
import org.adempiere.webui.trees.WTreeOnDropListener;
import org.adempiere.webui.util.TreeUtils;
import org.compiere.util.Env;
import org.compiere.util.Msg;
import org.zkoss.zk.ui.event.Event;
...
public class ADTreePanel extends Panel implements EventListener
{
    ...
    public void initTree(int AD_Tree_ID, int windowNo) 
    {
    	//Load the tree (like MTree in swing client). The tree loads its root node by itself
    	WTree wTree = new WTree (Env.getCtx(), AD_Tree_ID, true, false, false, null);
    	...
    	model.addOnDropEventListener(new WTreeOnDropListener(tree, wTree, model, windowNo));
    	    	
    	//Set Trees Model/CellRenderer
    	tree.setModel(model);
    	tree.setTreeitemRenderer(model);
    	
    	//Init the SearchPanel
