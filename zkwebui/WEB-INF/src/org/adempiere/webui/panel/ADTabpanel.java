...
import org.adempiere.webui.editor.WebEditorFactory;
import org.adempiere.webui.event.ContextMenuListener;
import org.adempiere.webui.trees.WTree;
import org.adempiere.webui.trees.WTreeModel;
import org.adempiere.webui.trees.WTreeNode;
import org.adempiere.webui.util.GridTabDataBinder;
import org.adempiere.webui.window.FDialog;
import org.compiere.model.DataStatusEvent;
import org.compiere.model.DataStatusListener;
...
DataStatusListener, IADTabpanel, VetoableChangeListener
{
            ...
            GridWindow gridWindow)
    {
        ...
        int AD_Tree_ID = 0;
		if (gridTab.isTreeTab())
			AD_Tree_ID = WTree.getDefaultAD_Tree_ID (Env.getAD_Client_ID(Env.getCtx()), gridTab.getKeyColumnName());
		if (gridTab.isTreeTab() && AD_Tree_ID != 0)
		{
			Borderlayout layout = new Borderlayout();
			layout.setParent(this);
    ...
    public void createUI()
    {
        ...
        //create tree
        if (gridTab.isTreeTab() && treePanel != null) {
			int AD_Tree_ID = WTree.getDefaultAD_Tree_ID (
				Env.getAD_Client_ID(Env.getCtx()), gridTab.getKeyColumnName());
			treePanel.initTree(AD_Tree_ID, windowNo);
        }
    ...
    public void onEvent(Event event)
    {
    	if (event.getTarget() == listPanel.getListbox())
    	{
    		...
    		Treeitem item =  treePanel.getTree().getSelectedItem();
    		if(item!=null)
    			navigateTo((WTreeNode)item.getValue());
    	}
    }

    private void navigateTo(WTreeNode node) {
    	//  We Have a TreeNode
		int nodeID = node.getNode_ID();
		//  root of tree selected - ignore
		//if (nodeID == 0)
			//return;
        ...
        if (treePanel != null) {
        	...
        	if ("Deleted".equalsIgnoreCase(e.getAD_Message())){
        		if (e.Record_ID != null && e.Record_ID instanceof Integer && ((Integer)e.Record_ID != gridTab.getRecord_ID()))
        			...
        			setSelectedNode(gridTab.getRecord_ID(), false);
        	}else if ("Saved".equalsIgnoreCase(e.getAD_Message())){
        		setSelectedNode(gridTab.getRecord_ID(), true);
        	}
        	else{
        		setSelectedNode(gridTab.getRecord_ID(), false);
        	}
    ...
    private void deleteNode(int recordId) {
		...
		WTreeModel model = (WTreeModel) treePanel.getTree().getModel();

		if (treePanel.getTree().getSelectedItem() != null) {
			WTreeNode node = (WTreeNode) treePanel.getTree().getSelectedItem().getValue();
			if (node.getNode_ID() == recordId) {
				model.removeNode(node);
				return;
			}
		}

		WTreeNode node = model.findNode(recordId, treePanel.getTree());
		if (node != null) {
			model.removeNode(node);
		}
	}
	...
	private void addNewNode() {
    	if (gridTab.getRecord_ID() > 0) {
			...
			//
			WTreeModel model = (WTreeModel) treePanel.getTree().getModel();
			WTreeNode root = (WTreeNode)model.getRoot();
			WTreeNode node = new WTreeNode (root.getWTree(), gridTab.getRecord_ID(), 0, name, description,
					root.getNode_ID(), summary, imageIndicator, false, null);
			model.addNode(node);
			int[] path = model.getPathFromRootToNode(node);
			Treeitem ti = treePanel.getTree().renderItemByPath(path);
			treePanel.getTree().setSelectedItem(ti);
    	}
	}

	private void setSelectedNode(int recordId, boolean isSaveEvent) {
		
		//If no record-id, return
		if (recordId <= 0) return;

		//if an item is selected, we either have to update it or do nothing
		if (treePanel.getTree().getSelectedItem() != null) {
			
			//First load the selected node
			WTreeNode treeNode = (WTreeNode) treePanel.getTree().getSelectedItem().getValue();
			
			//If the node is selected and we have a save event, we want to update the node
			if (treeNode.getNode_ID() == recordId && isSaveEvent) {
				...
				treeNode.setSummary(summary);
				treeNode.setImageIndicator(imageIndicator);
								
				//Get the model
				WTreeModel model = (WTreeModel) treePanel.getTree().getModel();
				
				//Tell the model the node was updated
				model.nodeUpdated(treeNode);
				
				return;
			}
			
			//If the node is selected and we have no save event, we want to do nothing. just return
			if (treeNode.getNode_ID() == recordId && !isSaveEvent) {
				return;
			}
		}
		...
		WTreeModel model = (WTreeModel) treePanel.getTree().getModel();
		WTreeNode treeNode = model.findNode(recordId, treePanel.getTree());
		if (treeNode != null) {
			int[] path = model.getPathFromRootToNode(treeNode);
			Treeitem ti = treePanel.getTree().renderItemByPath(path);
			treePanel.getTree().setSelectedItem(ti);
		} 
		else if(treeNode == null && isSaveEvent){
			addNewNode();
		}
	}
