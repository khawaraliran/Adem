...
public class MenuPanel extends Panel implements EventListener
{
    ...
    public MenuPanel()
    {
        ...
        //For the TreeSearchPanel we need a WTree!
        WTree wTree = new WTree (Env.getCtx(), adTreeId, false, true, false, null);
        wTree.trimTree();
        
        //Get the trees root node
    	WTreeNode root = wTree.getRoot();
        
    	//Create TreeModel from root node
    	WTreeModel model = new WTreeModel(root);
    	
    	//Set models WTree and DropEventListener
    	model.setWTree(wTree);
    	model.setItemDraggable(false);
    	model.addOnClickEventListener(this);
    	model.setLeafsDraggable("favourite");
    	
    	//Set Trees Model/CellRenderer
    	menuTree.setModel(model);
    	menuTree.setTreeitemRenderer(model);
    	
    	//Sets the TreeSearchPanels Model
        pnlSearch.setWTreeModel(model);
    }
    ...
    /** Just a little helper to get the Treecell for a given Treeitem (this method is stolen form Treeitem but it is private...
     * Jan Thielemann - jan.thielemann@evenos.de - evenos GmbH
     */
//    private Treecell treecellForTreeitem(Treeitem treeitem) {
...
//			cell.setParent(treeitem.getTreerow());
//		}
//		return cell;
//	}
    ...
    { 
        ...
        if (eventName.equals(Events.ON_CLICK)) {
    		...
    		WTreeNode node = (WTreeNode)selectedItem.getValue();
    		if(node != null && !node.isSummary())
            {
                fireMenuSelectedEvent(node);
            }
            else
            {
            	selectedItem.setOpen(!selectedItem.isOpen());
    ...
    protected void fireMenuSelectedEvent(WTreeNode node) {
    	try
        {
    		SessionManager.getAppDesktop().onMenuSelected(node.getNode_ID());
        }
        catch (Exception e)
        {
            throw new ApplicationException(e.getMessage(), e);
