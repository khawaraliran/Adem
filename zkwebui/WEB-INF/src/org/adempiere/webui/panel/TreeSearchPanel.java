...
import org.adempiere.webui.trees.WTreeAutoComplete;
...
public class TreeSearchPanel extends Panel implements EventListener, TreeDataListener
{
    ...
    private Label lblSearch;
    private WTreeAutoComplete wTreeAutoCompleteSearch;
    ...
    private void init()
    {
        ...
        wTreeAutoCompleteSearch = new WTreeAutoComplete();
        wTreeAutoCompleteSearch.setAutodrop(true);
        wTreeAutoCompleteSearch.addEventListener(Events.ON_CHANGE, this);
        if (AEnv.isInternetExplorer())
        {
        	wTreeAutoCompleteSearch.setWidth("200px");
        }

        this.appendChild(div);
        this.appendChild(wTreeAutoCompleteSearch);
        if (!AEnv.isInternetExplorer())
        {
        	this.setStyle("height: 20px;");
    	}
    ...
    public void onEvent(Event event)
    {
        if (wTreeAutoCompleteSearch.equals(event.getTarget()) && (event.getName().equals(Events.ON_CHANGE)))
        {
        	
            String value = wTreeAutoCompleteSearch.getValue();
            		...
            		&& value.substring(0, 1).equals(PREFIX_DOCUMENT_SEARCH))
            {
            	DocumentSearch search = new DocumentSearch();
            	if (search.openDocumentsByDocumentNo(value.substring(1)))
            		wTreeAutoCompleteSearch.setText(null);
            	return;
            }
            
            /* TODO: Sometimes it can happen that the user types in a text and hits return before the autocompletion field was filled.
             * in this case, the comboitem == null. it would be a good idea to start a new search in this case so we can be sure there
             * is really no possible entry to select! 
             */
            Comboitem item = wTreeAutoCompleteSearch.getSelectedItem();
            if(item!=null){

            	HashMap<String, Object> nodeMap = (HashMap<String, Object>) item.getValue();
            	if(nodeMap != null){
            		
                	wTreeAutoCompleteSearch.setEnabled(false);

                	selectItem(nodeMap);
                	
                	wTreeAutoCompleteSearch.setEnabled(true);
                	
            	}
            }else{
            	//No entry selected and text was not used by DocumentSearch - we don't have anything for this so we don't need so search in the database
            	return;
            }
        }
    }
    ...
    private void selectItem(HashMap<String, Object> nodeMap){
	
    	Clients.showBusy(Msg.getMsg(Env.getCtx(), "Loading"), true);
    	
    	WTreeNode node = m_wmodel.findNode((Integer)nodeMap.get("node_id"), tree);
    	
    	int[] path = m_wmodel.getPathFromRootToNode(node);
    	
    	Treeitem item = tree.renderItemByPath(path);
	...
	public void setWTreeModel(WTreeModel model){
		m_wmodel = model;
		if(wTreeAutoCompleteSearch!=null){
			wTreeAutoCompleteSearch.setWTreeModel(model);
		}
	}
