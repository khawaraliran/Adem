...
public class DPFavourites extends DashboardPanel implements EventListener {
	...
	public DPFavourites()
	{
		...
		Image img = new Image("/images/Delete24.png");
		//Image img = new Image();
		//img.setContent(Env.getRenderedImage("Delete", 24));
		favToolbar.appendChild(img);
		img.setAlign("right");
		img.setDroppable(DELETE_FAV_DROPPABLE);
		img.addEventListener(Events.ON_DROP, this);
	...
	//TODO: Auf WTree trefactoren + richtige Icons laden!!!
	private Box createFavouritesPanel()
	{
		bxFav = new Vbox();
		...
		WTree wTree = new WTree (Env.getCtx(), AD_Tree_ID, false, true, false, null);
        wTree.trimTree();

        addFavouritesRecursiveFromNode(wTree.getRoot());
	...
	private void addFavouritesRecursiveFromNode(WTreeNode node){
		if(node.isSummary()){
			for(WTreeNode nd : node.getChildren()){
				if(nd.isSummary()){
					addFavouritesRecursiveFromNode(nd);
				}else{
					if (nd.isOnBar()) {
						String label = nd.toString().trim();
						ToolBarButton btnFavItem = new ToolBarButton(String.valueOf(nd.getNode_ID()));
	...
	public void addItem(Treeitem treeitem) {
		...
		if(value != null)
		{
			int Node_ID = ((WTreeNode)value).getNode_ID();
			if(barDBupdate(true, Node_ID))
			{
				String label = treeitem.getLabel().trim();
				ToolBarButton btnFavItem = new ToolBarButton(String.valueOf(Node_ID));
	...
	private String getIconFile(WTreeNode mt) {
		if (mt.isWindow())
			return "/images/mWindow.png";
		if (mt.isReport())
			return "/images/mReport.png";
