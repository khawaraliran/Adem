	...
	private void action_treeAdd(ListItem item)
	{
		...
		if (item != null)
		{
			...
			if (stn != null) {
				...
				stn = new SimpleTreeNode(new MTreeNode(((MTreeNode)model.getRoot().getData()).getMTree(), item.id, 0, item.name, item.description, 0, item.isSummary,
						item.imageIndicator, false, null), new ArrayList<Object>());
				model.addNode(stn);
			}
			//	May cause Error if in tree
