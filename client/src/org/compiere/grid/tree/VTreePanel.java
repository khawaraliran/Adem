...
//			if (search.openDocumentsByDocumentNo(treeSearch.getText().substring(1)))
		...
		else if (e.getSource() == treeSearch)
		{
				...
				if(searchText!=null && searchText.length()>0){
					...
					VSearchTreeNodeWorker worker = new VSearchTreeNodeWorker(this, searchText);
					worker.start();
				}
		}   //  treeSearch
		...
		String name, String description, boolean isSummary, String imageIndicator)
	{
		...
		{	
			...
			//Add new node reference to searchList
			//addNodeToSearchList(node.getName(), node.getDescription(), node.getParent_ID(), node.getNode_ID());
		}
		...
		else if (node != null && save)
		{
			...
			//Update node reference in searchList
			//updateNodeInSearchList(node.getName(), node.getDescription(), node.getParent_ID(), node.getNode_ID());
		}
