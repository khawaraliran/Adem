 ...
 *
 *  @author 	Jorg Janke
 *  @version 	$Id: AEnv.java 1009 2012-02-09 09:16:13Z suman $
 *  
...
public static final String svnRevision =  "$Id: AEnv.java 1009 2012-02-09 09:16:13Z suman $";
	/**
	...
	*/ 
	...
	public static void updateLogmig(){
		
		ArrayList<Container>	sw_windows = Env.getWindows();
		for (int i = 0; i < sw_windows.size(); i++)
		{
			Container cmp = (Container)sw_windows.get(i);
			if (cmp != null && cmp instanceof org.compiere.apps.APanel)
			{
				org.compiere.apps.AppsAction apc = ( (org.compiere.apps.APanel)cmp ).getLogmig(); 
				if( Env.getContext(Env.getCtx(), EagleConstants.LOG_MIGRATION ).equals("Y")){
					apc.setPressed( Boolean.TRUE );
				}else{
					apc.setPressed( Boolean.FALSE );
				}
			}
		}
	}
