 ...
 *  @version $Id: MOrder.java 1157 2012-03-16 04:43:37Z yadagiri $
 * 
 ...
 *  @BugNo: 1621 
 *  @changes : in createOrderTaxes method calculate tax with passing the wtc_formType_ID selected at order level
 *  
 *   *	<pre>
 *
 *<li>  @author         @Identifier         @BugNo		@ChangeDescription
 *<li> D.Yadagiri Rao   201203161015         2591		Modified getShipments() method now we getting only active records only
 *  
 *  </pre>
 *  
 *  
 ...
 */
public static final String svnRevision =  "$Id: MOrder.java 1157 2012-03-16 04:43:37Z yadagiri $";
	/**
	 * 
	 */
	...
	/**
	 *	201203161015	----> Getting active shipments of order 	
	 *
	 ...
	 *  Deletes the OrderTax's created by the invoice lines and recalculates the Order taxes.
	 *  
	 *  create InvoiceTax for the tax selected at line including the tax amounts of all the child taxes of the tax
	 *  selected at  line.
	 *  
	 *  Tax at  line is document level Single invoice tax for the whole Document.
	 *  
	 *  All the OrderTax's created for the invoice lies will be again split into the
	 *  number of the child taxes for the tax at line.
	 *  
	 *  inital OrderTax created for each line will be deleted.
	 *   
	 *    
	...
	public boolean calculateTaxTotal()
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (!documentLevelTaxes.contains(taxID))
			{
				...
				if( MTax.get(getCtx(), taxID ).isDocumentLevel() ){
		...
		for (int i = 0; i < taxes.length; i++)
		{
			...
			{ 
				BigDecimal taxAmt = this.createOrderTaxes( tax, oTax , Boolean.FALSE );
				...
				//				}
				oTax.deleteEx(true, get_TrxName());
	...
	/**
	 * Create's Order taxes for all the child's of the tax selected at  line .
	 * @param cTax -- Tax at line
	 * @param oTax -- MOrderTax created for the tax selected at line ( this tax amount should include the tax
	 ...
	 * @return
	 */
	private BigDecimal createOrderTaxes( MTax cTax , MOrderTax oTax , boolean baseTaxExists){
		
		BigDecimal taxAmt = Env.ZERO;
		MTax[] childTaxes = new MTax[0];
		HashMap<Integer, SummaryOrderTax> taxBackup = new HashMap<Integer, SummaryOrderTax>();
		...
		if( cTax.getTaxType().equalsIgnoreCase( X_C_Tax.TAXTYPE_Summary ) ){
			...
			for( MTax tax : childTaxes ){
				...
				if( ! TaxUtil.containsTaxWithSource(tax, childsWithBasePlusTax ) ) {
					...
					taxAmt = taxAmt.add( this.createOrderTaxes( tax, oTax ,baseTaxExists ) );
					SummaryOrderTax sot = new SummaryOrderTax();
					sot.setC_Tax_ID(tax.get_ID());
					sot.setTaxAmount( taxAmt );
					sot.setTaxBaseAmt( oTax.getTaxBaseAmt() );
					taxBackup.put( tax.get_ID(), sot );
				}
			}
			
			for( MWTCChildTax child : childsWithBasePlusTax ){
				
				SummaryOrderTax sit = taxBackup.get( child.getSource_Tax_ID() );
				MOrderTax parentOrderTax = new MOrderTax(getCtx(), 0, get_TrxName() );
				parentOrderTax.setC_Tax_ID( sit.getC_Tax_ID() );
				parentOrderTax.setTaxAmt( sit.getTaxAmount() );
				parentOrderTax.setTaxBaseAmt( sit.getTaxBaseAmt() );
				parentOrderTax.setC_OrderLine_ID( oTax.getC_OrderLine_ID() );
				
				taxAmt = taxAmt.add( this.createOrderTaxes( ( (MTax)child.getChild_Tax() ) , parentOrderTax , Boolean.TRUE ) );
				parentOrderTax = null;
			}
		} else if( cTax.getTaxType().equalsIgnoreCase( X_C_Tax.TAXTYPE_Normal ) ) {
			
			BigDecimal baseAmt = oTax.getTaxBaseAmt();
			if( baseTaxExists ){
				BigDecimal baseTaxAmt = ( (MTax) oTax.getC_Tax()).calculateTax( baseAmt , Boolean.FALSE , getPrecision() , Boolean.TRUE , this.getWTC_FormType_ID() ); //BugNo: 1621 
				baseAmt = baseAmt.add(  baseTaxAmt );
			}
			childTaxes = cTax.getChildTaxes( Boolean.FALSE );
			taxAmt = taxAmt.add( cTax.calculateTax( baseAmt , Boolean.FALSE , getPrecision() , baseTaxExists , this.getWTC_FormType_ID()) ); //BugNo: 1621 
			MOrderTax newOTax = this.createOrderTax(cTax, baseAmt , taxAmt, oTax.getC_OrderLine_ID() );
			
			for( MTax tax : childTaxes ){
				taxAmt = taxAmt.add( this.createOrderTaxes( tax, newOTax , Boolean.FALSE ) );
			}
			
		} else {
			//BugNo: 1621 
			taxAmt = taxAmt.add( cTax.calculateTax( oTax.getTaxAmt(),Boolean.FALSE , getPrecision() , Boolean.FALSE ,this.getWTC_FormType_ID()  ) );
			this.createOrderTax( cTax, oTax.getTaxAmt(), taxAmt, oTax.getC_OrderLine_ID() );
		}
	 ...
	 * @return
	 */
	private MOrderTax  createOrderTax( MTax cTax, BigDecimal baseAmt , BigDecimal taxAmt , int lineId ){
		...
		newOTax.setTaxBaseAmt( baseAmt );
		newOTax.setTaxAmt(taxAmt);
		newOTax.setC_OrderLine_ID( lineId ); 
		
		if (!newOTax.save(get_TrxName())){
			log.saveError( ""," Fialed to create OrderTax for " + cTax );
			return null;
		}else { 
			return newOTax ;
		}
	...
	/**
	 * Class maintains the data of the summary Order taxes.
	 * which will be used for the base+tax type of child tax.
	 * @author PhaniKiran.Gutha
	 *
	 */
	private class SummaryOrderTax{
