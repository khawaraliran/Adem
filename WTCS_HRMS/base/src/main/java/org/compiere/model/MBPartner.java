 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MBPartner.java 1009 2012-02-09 09:16:13Z suman $
 * 
 ...
 *--------------------------------------------------------------------------------- 
 *1655				25/12/2011		251220111159		Ranjit			New entry in hr_employee_weeklyoff table 
 *																		if employee has specific weekly off policy
 *
 *1655				6/1/2012		201201060710		Ranjit			When department id changed then updating HR_Employee table
 *
 */
public class MBPartner extends X_C_BPartner
{
// Kindly do not delete below line as it is being used for svn version maintenance
public static final String svnRevision =  "$Id: MBPartner.java 1009 2012-02-09 09:16:13Z suman $";
	/**
	 * 
	 */
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if(newRecord && isEmployee()) {
			
			
			int count  = checkForUniqunessEmployeeCode(this);
			
			if( count > 0 ) {
				
				String msg = Msg.getMsg( getCtx(),EagleConstants.UNIQUE_EMPLOYEE_CODE_MSG);
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		//
		// Create an employee record for the business partner
		//
		
		if ( success && ( isEmployee() == true ) ) {
			
			success = createEmployee( newRecord );
		}
		...
		if( newRecord || (success && is_ValueChanged(COLUMNNAME_EMail))) {
			
			MUser user = getUserForBusinessPartner(this.getC_BPartner_ID(),get_TrxName());
			...
			if(null != user) {
				...
				if(!user.save()) {
					...
					log.saveError("Error","Failed to save the email for the user - " + this.getName());
		...
		// 251220111159 [Change Identification]
		
		if(isEmployee() && iscompanyweekpolicy() == Boolean.FALSE) {

			success = createEmployeeWeeklyOffRecord(this);
		}

		// 201201060710
		
		if(!newRecord && this.isEmployee() && is_ValueChanged(I_C_BPartner.COLUMNNAME_HR_Department_ID) ) {

			 success = updateDepartmentForEmployee(this);
			 
			 if(Boolean.FALSE == success) {
				 
				 String msg = Msg.getMsg( getCtx(),EagleMessageConstants.DEPARTMENT_UPDATE_FAIL_MSG);
				 log.saveError("Error", msg);
	...
	/**
	 * 201201060710 [Change Identification]
	 * 
	 * <P>
	 * Updates the department to the HR_Employee table when <BR>
	 * department for employee changed
	 * </P>
	 * 
	 * 
	 * @param mbPartner	: MBPartner
	 * @return			: TRUE if updated successfully else FALSE
	 */
	private boolean updateDepartmentForEmployee(MBPartner mbPartner) {
		...
		MHREmployee employee = MHREmployee.getActiveEmployee( getCtx(), mbPartner.getC_BPartner_ID(), get_TrxName());
		...
		employee.setHR_Department_ID(mbPartner.getHR_Department_ID());
		
		if(!employee.save(get_TrxName())) {
	...
	/**
	 * If record already exist then no need to create .
	 * 
	 * @param mbPartner
	 * @return
	 */
	private boolean createEmployeeWeeklyOffRecord(MBPartner mbPartner) {
	
		String sql = "SELECT count(*) FROM HR_Employee_WeeklyOff WHERE C_BPartner_ID = ? AND isactive = 'Y'";
		
		int count = DB.getSQLValue(null, sql, mbPartner.getC_BPartner_ID());
		
		if(count < 1) {
		
			X_HR_Employee_WeeklyOff employee_WeeklyOff = new X_HR_Employee_WeeklyOff(Env.getCtx(), 0, get_TrxName());
		
			employee_WeeklyOff.setC_BPartner_ID(mbPartner.getC_BPartner_ID());
			employee_WeeklyOff.setDescription(EagleConstants.EMPLOYEE_WEEKLYOFF_DESCRIPTION);
		
			if(!employee_WeeklyOff.save()) {
			
				log.log(Level.SEVERE,"Failed to create an employee individual weekly off record");
				return Boolean.FALSE;
			}
		}else {
			
			log.log(Level.FINE,"Employee has already weekly off entry - [BP Employee - " + mbPartner.getC_BPartner_ID() + "]");
		}
		return Boolean.TRUE;
		
	}

	public static MUser getUserForBusinessPartner(int c_BPartner_ID,String trxName) {
		
		StringBuffer whereClause = new StringBuffer( I_AD_User.COLUMNNAME_C_BPartner_ID + " = ?"); 
		
		MUser user = new Query(Env.getCtx(), I_AD_User.Table_Name, whereClause.toString(), trxName).
	...
	/**
	 * Creates the employee for the Business Partner
	 * 
	 * @param newRecord	: True if new record else false
	 * @return			: True if employee created successfully else False
	 */
	
	private boolean createEmployee( boolean newRecord ) {
		
		X_HR_Employee employeeModel = null;

		if (true == newRecord) {
		
			employeeModel = new X_HR_Employee( this.getCtx(), 
											   0,
											   this.get_TrxName());
		} else {
			
			int employeeId = this.getEmployeeIdForBPartner( this.getC_BPartner_ID());
		...
		employeeModel.setC_BPartner_ID(this.getC_BPartner_ID());
		employeeModel.setHR_Department_ID(this.getHR_Department_ID());
		employeeModel.setStartDate(this.getjoiningdate());
		employeeModel.setEndDate(this.getrelievingdate());

		boolean success = employeeModel.save();

		if ( true == success && true == newRecord ) {
			
			success = this.createAccomodationHistory();														// Creates employee quarter allocation details
			success = LeaveTypeManager.allocateLeavesForEmployee(this, get_TrxName());						// Assign the employee leaves

		} else if (true == success) {

			// Find the last entry in the quarter allocation table
			// update this entry with current date
			//
			StringBuffer whereClause = new StringBuffer();
			whereClause.append(X_HR_Emp_Acomm_History.COLUMNNAME_C_BPartner_ID)
					.append(" = " + this.getC_BPartner_ID());
			whereClause.append(" AND todate IS NULL ");

			Query query = new Query(this.getCtx(),
					X_HR_Emp_Acomm_History.Table_Name, whereClause.toString(),
					this.get_TrxName());
			List<X_HR_Emp_Acomm_History> list = query.list();

			if (list != null && list.size() > 0) {
				X_HR_Emp_Acomm_History lastActiveAccomRecord = list.get(0);

				if (lastActiveAccomRecord.getHR_Quarter_ID() != this
						.getHR_Quarter_ID()
						|| lastActiveAccomRecord.getHR_Block_ID() != this
								.getHR_Block_ID()) {

					Timestamp timestampYesterday = new Timestamp(Calendar
							.getInstance().getTimeInMillis()
							- EagleConstants.MILI_SECONDS_IN_DAY);
					lastActiveAccomRecord.setToDate(timestampYesterday);

					success = lastActiveAccomRecord.save();

					if (true == success && this.getHR_Block_ID() > 0
							&& this.getHR_Quarter_ID() > 0) {
						//
						// The new quarter is being allocated to the employee
						//
						success = this.createAccomodationHistory();
					}
				}

			} else {
				//
				// The quarter is being assigned for the first time.
				//
				success = this.createAccomodationHistory();
			}
		}
	...
	/**
	 * Gets the Employee ID for the Business partner
	 * 
	 * @param bpartnerId	: Business Partner ID
	 * @return				: Employee ID
	 */
	private int getEmployeeIdForBPartner( int bPartnerId ) {
		int employeeId = 0;
		
		String sql1 = "SELECT HR_EMployee_ID FROM HR_EMployee WHERE c_bpartner_id =? ";
		employeeId = DB.getSQLValue(null, sql1, bPartnerId); 
		
		return employeeId;
	}

	/**
	 * Creates the entry for the employee accommodation details
	 *  
	 * @return	: TRUE if creates entry successfully else FALSE
	 */
	private boolean createAccomodationHistory() {

		boolean success = true;

		if ( this.getHR_Block_ID() > 0 && 
			 this.getHR_Quarter_ID() > 0) {
			
			Timestamp timestampToday = new Timestamp(Calendar.getInstance().getTimeInMillis());

			X_HR_Emp_Acomm_History empAcommHistory = new X_HR_Emp_Acomm_History( this.getCtx(), 
																				 0, 
																				 this.get_TrxName());
			empAcommHistory.setC_BPartner_ID(this.getC_BPartner_ID());
			empAcommHistory.setHR_Block_ID(this.getHR_Block_ID());
			empAcommHistory.setHR_Quarter_ID(this.getHR_Quarter_ID());
			empAcommHistory.setFromDate(timestampToday);
			empAcommHistory.setToDate(null);

			success = empAcommHistory.save();
			
			if( Boolean.FALSE == success ) {
				
				log.log( Level.SEVERE, 
						 "Failed to create the accomodation hisotory for the employee - " 
						+ this.get_ID());
			}
		}
	...
	/**
	 * Creates the history for the name of the vendor
	 * 
	 * @param vendor
	 */
	
	private void createVendorNameHistory( MBPartner vendor ) {
		
		X_ES_Partner_Name_History history = new X_ES_Partner_Name_History( this.getCtx(), 
																		   0 ,
																		   this.get_TrxName());
		history.setC_BPartner_ID(this.getC_BPartner_ID());
		history.setName(this.getName());
		history.setFromDate(this.getCreated());
		history.setToDate(new Timestamp(Calendar.getInstance().getTimeInMillis()));
		
		if(!history.save())	{
			
			log.log(Level.SEVERE, "History Maitenece is Not Happen For This Record");
		}
	}
	
	private int checkForUniqunessEmployeeCode(MBPartner employee) {

		StringBuffer whereClause = new StringBuffer( I_C_BPartner.COLUMNNAME_employee_code + " =? ");
		
		int count  = new Query( Env.getCtx(), I_C_BPartner.Table_Name, whereClause.toString(), get_TrxName()).
		setParameters(employee.getemployee_code().trim().toUpperCase()).
