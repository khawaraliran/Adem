 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MInOut.java 1157 2012-03-16 04:43:37Z yadagiri $
 *  
 ...
 *  
 *  @author				         @IssueNo	@ChangeID 			@Description
 *  PhaniKiran.Gutha	           1976		201201241124		Doctype should be modified based on the document base type only when minout is going to save with out the document type id
 *  D. Yadagiri Rao		           2038		201202031754		If DocBaseType is Material Delivery Then it is set Sales Transaction
 *  Yadagiri Rao.Dontineni         2591     201203161015        getting the active records from the Line based on the shipment	
 */
 ...
 */
public static final String svnRevision =  "$Id: MInOut.java 1157 2012-03-16 04:43:37Z yadagiri $";
	/**
	 * 
	 */
	...
	public MInOutLine[] getLines (boolean requery)
	{
		...
		//
		//	201203161015
		//	Get the active Line records of shipment
		//
	...
	public MInOutConfirm[] getConfirmations(boolean requery)
	{
		...
		String sql = "SELECT * FROM M_InOutConfirm WHERE M_InOut_ID=? AND IsActive= 'Y'";
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		// 
		// 201202031754
		// If it docbasetype is material Delivery Then it is set as Sales Transaction
		//
		
		if ( getC_DocType() != null
				&& getC_DocType().getDocBaseType().equals(
...
//	}
	...
	private void checkMaterialPolicy(MInOutLine line)
	{
		...
		if (product != null && line.getM_AttributeSetInstance_ID() == 0)
		{
			...
			Query query = MTable.get(getCtx(), MInOutLineMA.Table_Name)
				.createQuery(I_M_InOutLineMA.COLUMNNAME_M_InOutLine_ID+"=?", get_TrxName() );
				query.setParameters( line.getM_RMALine().getM_InOutLine_ID() );
				query.setOnlyActiveRecords( Boolean.TRUE );
				List<MInOutLineMA> list = query.setOrderBy( MInOutLineMA.COLUMNNAME_M_AttributeSetInstance_ID + " ASC ").list();
				
				List<LineMA> retMA =  this.getReturnedQty(line);
				BigDecimal movQty = line.getMovementQty();
				for( MInOutLineMA linema : list ){
					BigDecimal alocQty = linema.getMovementQty();
					Boolean found = Boolean.FALSE;
					for( LineMA lma : retMA ){
					...
					if( alocQty.signum() == 0 )
						continue;
					MInOutLineMA ma = new MInOutLineMA( line, linema.getM_AttributeSetInstance_ID(), alocQty );
					ma.saveEx();
	...
	private List<LineMA> getReturnedQty(MInOutLine line ){
//		BigDecimal retVaue = Env.ZERO;
		List<LineMA> mas = new ArrayList<MInOut.LineMA>();
		int refid = line.getM_RMALine().getM_InOutLine_ID();
		String sql = "SELECT SUM(lma.movementqty),lma.m_attributesetinstance_id FROM M_InoutLine mil " +
				" JOIN M_InoutLineMA lma  ON ( mil.M_InoutLine_ID = lma.M_inoutLine_ID) " +
				" JOIN M_RMALine rmal ON ( rmal.M_RMALine_ID = mil.M_RMALine_ID ) " +
				" JOIN M_Inout min ON ( min.M_Inout_ID = mil.M_Inout_ID ) " +
				" WHERE ( rmal.M_inoutLine_ID = ? ) " +
				" AND  min.DocStatus IN ('CO','CL')  " +
				" AND min.movementtype ='C+' GROUP BY lma.m_attributesetinstance_id "; 
	...
	/**
	 * Create Cost Detail from transaction
...
/*	private void createCostDetail(MTransaction mtrx, MInOutLine sLine)
	{
		...
		if (sLine.getReversalLine_ID() > 0)
		{
			final String whereClause = MCostDetail.COLUMNNAME_M_InOutLine_ID+"=?"
					+" AND "+MCostDetail.COLUMNNAME_M_AttributeSetInstance_ID+"=?";
			List<MCostDetail> list = new Query(mtrx.getCtx(), MCostDetail.Table_Name, whereClause, get_TrxName())
			.setParameters(new Object[]{sLine.getReversalLine_ID(), mtrx.getM_AttributeSetInstance_ID()})
			.list();
			for (MCostDetail cd : list)
			{
				MCostDetail cdnew = new MCostDetail(getCtx(), 0, get_TrxName());
				copyValues(cd, cdnew);
		...
		{
			for(MAcctSchema as : MAcctSchema.getClientAcctSchema(getCtx(), mtrx.getAD_Client_ID()))
			{
				BigDecimal amt = sLine.getPriceActual().multiply(mtrx.getMovementQty());
