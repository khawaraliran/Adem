 ...
 *
 *  @author Jorg Janke
 *  @version $Id: MInvoice.java 1157 2012-03-16 04:43:37Z yadagiri $
 ...
 *  @BugNo: 1621 
 *  @changes 
 *  <li> Tax Calculation with the form type </li>
 *  <li> beforesave when form type selected at invoice then form status should be Expected </li>
 *  <li> Introduced new method isDebitNote </li>
 *    <li> Invoice tax calculation 
 *             Invoce taxes should be prepared for each and individual line and the taxes in the line.</li>
 *   <li> for AP/AR Tax Debit Note do not calculate tax total again and donot consider lines</li>
 *   
 *   
 *   @IssueNo			@Author				  @ChangeID		@Descriptoin
 *   1759           PhaniKiran.Gutha          201112947      Invoice getting created from order should copy the form type from order
 *   2591           Yadagiri Rao.Dontineni    201203161015   Using Tranassaction  For Junit testcase
 */
 ...
 */
public static final String svnRevision =  "$Id: MInvoice.java 1157 2012-03-16 04:43:37Z yadagiri $";
	/**
	 * 
	 */
	...
	public MInvoice (MOrder order, int C_DocTypeTarget_ID, Timestamp invoiceDate)
	{
		...
		setWTC_FormType_ID( order.getWTC_FormType_ID() ); //201112947
	...
	public MInvoice (MInOut ship, Timestamp invoiceDate)
	{
		...
		if( ship.getC_Order_ID() != 0 ){
			setWTC_FormType_ID( ship.getC_Order().getWTC_FormType_ID() ); //201112947
		}
	}	//	MInvoice
	...
	public void setC_DocTypeTarget_ID (String DocBaseType)
	{
		String sql = "SELECT C_DocType_ID FROM C_DocType "
			+ "WHERE AD_Client_ID=?  AND DocBaseType=?" // AND AD_Org_ID in (0,?) removed this line to support multiple calendar for org as part of Cost Engine
			+ " AND IsActive='Y' "
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//BugNo 1621
		// form type exists then Formstatus should be Expected
		
		if( is_new() || is_ValueChanged( I_C_Invoice.COLUMNNAME_WTC_FormType_ID ) ){
			
			if( getWTC_FormType_ID() != 0 ){ 
				
				setFormStatus( X_C_Invoice.FORMSTATUS_Expected );
	...
	public String prepareIt()
	{
		...
		if (lines.length == 0 && !isTaxDebitNote() )
		{
		...
		explodeBOM();
		if ( !isTaxDebitNote() && !calculateTaxTotal())	//	setTotals // BugNo 1621 donot calculate taxex again if the doctype is ARTaxDebitNote
		{
		...
		if (isSOTrx() && !isReversal())
		{
			//
			//	201203161015
			//	Using Tranassaction  For Junit testcase 
			//
	 ...
	 *  Deletes the invoice taxes created by the invoice lines and recalculates the invoice taxes.
	 *  
	 *  create InvoiceTax for the tax selected at line including the tax amounts of all the child taxes of the tax
	 *  selected at invoice line.
	 *  
	 *  Tax at invoice line is document level Single invoice tax for the whole invoice.
	 *  
	 *  All the invoice taxes created for the invoice lies will be again split into the
	 *  number of the child taxes for the tax at line.
	 *  
	 *  inital invoice tax created for each line will be deleted.
	 *   
	 *    
	...
	public boolean calculateTaxTotal()
	{
		...
		ArrayList<Integer> documentLevelTaxes = new ArrayList<Integer>();
		MInvoiceLine[] lines = getLines(false);
		for (int i = 0; i < lines.length; i++)
		{
			MInvoiceLine line = lines[i];
			Integer taxID = line.getC_Tax_ID();
			// document level tax to calculate only once for the invoice
			if (!documentLevelTaxes.contains(taxID))
			{
				MTax tax = (MTax) line.getC_Tax();
				// add the taxid to the list if the tax is document level
				if( tax.isDocumentLevel() ){
					documentLevelTaxes.add(taxID);
				}
		...
		for (int i = 0; i < taxes.length; i++)
		{
			...
			if (  tax.isSummary() || 
					tax.getTaxType().equalsIgnoreCase( X_C_Tax.TAXTYPE_Summary ) || 
					tax.getTaxType().equalsIgnoreCase( X_C_Tax.TAXTYPE_Normal ) )
			{
				// create individual invoicetax for the child taxes
					BigDecimal taxAmt = this.createInvoiceTaxes(tax, iTax,  Boolean.FALSE );
	...
	/**
	 * BugNo 1621
	 * this method indicates that the invoice is either a AP/AR Tax Debit Note
	 * @return True if target document type of the invoice is either AP/AR Tax Debit Note else false
	 */
	public boolean isTaxDebitNote(){
		
		int arTaxDebitNote = TaxUtil.getDocType( getAD_Client_ID(), getCtx(), EagleConstants.AR_TAX_DEBIT_NOTE );
		int apTaxDebitNote = TaxUtil.getDocType( getAD_Client_ID(), getCtx(), EagleConstants.AP_TAX_DEBIT_NOTE );
		
		if( arTaxDebitNote == getC_DocTypeTarget_ID() || 
				apTaxDebitNote == getC_DocTypeTarget_ID() ){
	...
	public String completeIt()
	{
		...
		// POS supports multiple payments , brought here as part of Costing engine 
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	/**
	 * Create's Invoice taxes for all the child's of the tax selected at invoice line .
	 * @param cTax -- Tax at line
	 * @param iTax -- MInvoiceTax created for the tax selected at line ( this tax amount should include the tax
	 *                amount of all the child taxes. base amount of this invoice tax will be used as the source
	 *                amount for the child tax calculation
	 * @param baseTaxExists 
	 *                Boolean variable to represent base exists for this tax or not.
	 * @return
	 */
	private BigDecimal createInvoiceTaxes( MTax cTax , MInvoiceTax iTax , boolean baseTaxExists){
		
		BigDecimal taxAmt = Env.ZERO;
		MTax[] childTaxes = new MTax[0];
		HashMap<Integer, SummaryInvoiceTax> taxBackup = new HashMap<Integer, SummaryInvoiceTax>();
		
		if( cTax.getTaxType().equalsIgnoreCase( X_C_Tax.TAXTYPE_Summary ) ){
			
			
			childTaxes = cTax.getChildTaxes( Boolean.FALSE );
			
			List<MWTCChildTax>  childsWithBasePlusTax = TaxUtil.getChildWithBaseTax( cTax.get_ID() , getCtx() , get_TrxName() );
			
			for( MTax tax : childTaxes ){
				
				if( ! TaxUtil.containsTaxWithSource(tax, childsWithBasePlusTax ) ) {
					
					taxAmt = taxAmt.add( this.createInvoiceTaxes( tax, iTax , baseTaxExists ) );
					SummaryInvoiceTax sit = new SummaryInvoiceTax();
					sit.setC_Tax_ID(tax.get_ID());
					sit.setTaxAmount( taxAmt );
					sit.setTaxBaseAmt( iTax.getTaxBaseAmt() );
					taxBackup.put( tax.get_ID(), sit );
				}
			}
			
			for( MWTCChildTax child : childsWithBasePlusTax ){
				
				SummaryInvoiceTax sit = taxBackup.get( child.getSource_Tax_ID() );
				MInvoiceTax parentInvoiceTax = new MInvoiceTax(getCtx(), 0, get_TrxName() );
				parentInvoiceTax.setC_Tax_ID( sit.getC_Tax_ID() );
				parentInvoiceTax.setTaxAmt( sit.getTaxAmount() );
				parentInvoiceTax.setTaxBaseAmt( sit.getTaxBaseAmt() );
				parentInvoiceTax.setC_InvoiceLine_ID( iTax.getC_InvoiceLine_ID() );
				
				taxAmt = taxAmt.add( this.createInvoiceTaxes( ( (MTax)child.getChild_Tax() ) , parentInvoiceTax , Boolean.TRUE ) );
				parentInvoiceTax = null;
			}
		} else if( cTax.getTaxType().equalsIgnoreCase( X_C_Tax.TAXTYPE_Normal ) ) {
			
			BigDecimal baseAmt = iTax.getTaxBaseAmt();
			if( baseTaxExists ){
				BigDecimal baseTaxAmt = ( (MTax) iTax.getC_Tax()).calculateTax( baseAmt ,  Boolean.FALSE , getPrecision() , Boolean.TRUE , this.getWTC_FormType_ID());
				baseAmt = baseAmt.add(  baseTaxAmt );
			}
			childTaxes = cTax.getChildTaxes( Boolean.FALSE );
			taxAmt = taxAmt.add( cTax.calculateTax( baseAmt ,  Boolean.FALSE , getPrecision()   , Boolean.FALSE ,  this.getWTC_FormType_ID()) );
			MInvoiceTax newITax = this.createInvoiceTax(cTax, baseAmt , taxAmt, iTax.getC_InvoiceLine_ID() );
			
			for( MTax tax : childTaxes ){
				taxAmt = taxAmt.add( this.createInvoiceTaxes( tax, newITax , Boolean.FALSE ) );
			}
			
		} else {
			
			taxAmt = taxAmt.add( cTax.calculateTax( iTax.getTaxAmt(),  Boolean.FALSE , getPrecision() , Boolean.FALSE , this.getWTC_FormType_ID() ) ); 
			this.createInvoiceTax( cTax, iTax.getTaxAmt(), taxAmt, iTax.getC_InvoiceLine_ID() );
		}
	...
	/**
	 * Creates Invoice Tax for the invoice and tax
	 * @param cTax
	 * @param baseAmt
	 * @param taxAmt
	 * @param lineId
	 * @return
	 */
	private MInvoiceTax  createInvoiceTax( MTax cTax, BigDecimal baseAmt , BigDecimal taxAmt , int lineId ){
		...
		newITax.setTaxBaseAmt( baseAmt );
		newITax.setTaxAmt(taxAmt);
		newITax.setC_InvoiceLine_ID( lineId ); 
		
		if (!newITax.save(get_TrxName())){
			log.saveError( ""," Fialed to create Invoice Tax for " + cTax );
			return null;
		}else { 
			return newITax ;
		}
	...
	/**
	 * Class maintains the data of the summary invoice taxes.
	 * which will be used for the base+tax type of child tax.
	 * @author PhaniKiran.Gutha
	 *
	 */
	private class SummaryInvoiceTax{
