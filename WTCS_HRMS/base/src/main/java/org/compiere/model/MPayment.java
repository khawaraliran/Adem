 ...
 *  @version 	$Id: MPayment.java 1009 2012-02-09 09:16:13Z suman $
 *  
 *  
 *  @author Arunkumar
 *  @Date : [16/11/2011] 
 *  For Allocations Which Don't Have Invoice We Need Not To Check For Doc Type
 *  In Document Info If The Invoice is Not There Then WE Have To Place Charge ID 
 *  
 *  
 *  @author Arunkumar
 *  @Date : [17/11/2011] we can search With This Date For Tracking The Changes
 *   Added Methods:           
 *          1.     getChargeRelatedTaxIndicators(MPaymentAllocate)  
 *                -- This Method is responsible for Getting The Charge Related Tax Indicators  
 *          
 *          2.     getTaxIndicatorRelatedTaxReturns(X_WTC_TaxIndicator, BigDecimal, int) 
 *                -- This Method Is REsponsible For Getting the TaxReturns Forms With Respect To Given taxIndicator
	                 And Adjusts The Open Amount
 *           
 *          3.     createMWTCTaxretrunPaymentAlllocation(BigDecimal, int, int, int)     
 *                -- This Method Is Responsible For Creating WTC_TaxreturnPaymentAllocation Entry.
 * 			         This Table Will Hold The Data Related To Open Amount of Tax Retruns That Is Filled With Allocation Amount Amount Of The Charge
 * 
 * @author PhaniKiran.Gutha
 * <pre>
 * Date [06/12/2011]
 * 		<li> 1. Introduced new variable fromCreatePayment to indicate that payment is created from CreatePayment Form. </li> 
 *      <li> 2. While Completing the payment if the payment is created form the createpayment
 *       popup the do not allocate charge </li>
 *       
 *       </pre>
 *       
 *  @Bug     @author       @ChangeID  		  @Description    
 *  ------   -------       -------------   	   ------------
 *  1638     Arunkumar     [20111222:9:00]    With LOC , DiscountAmt,WithholdingAmt,WriteOffAmt Are Became Virtual Columns     
 * 							                  For Virtual Columns If We Try To Place Values It will through Illegal argument Error.
 *       
 */
...
public static final String svnRevision =  "$Id: MPayment.java 1009 2012-02-09 09:16:13Z suman $";
    ...
    private   boolean fromCreatePayment = Boolean.FALSE;



	public  boolean isFromcreatepayment() {
		return fromCreatePayment;
	}

	public void setFormCreatePayment( boolean formCretepayment ){
		
		fromCreatePayment = formCretepayment;
	}
	...
	public MPayment (Properties ctx, int C_Payment_ID, String trxName)
	{
		...
		if (C_Payment_ID == 0)
		{
			...
			//Bug-1638
			//[20111222:9:00]
			//With LOC , DiscountAmt,WithholdingAmt,WriteOffAmt Are Became Virtual Columns 
			//For Virtual Columns If We Try To Place Values It will through Illegal argument Error.
...
*/
	...
	{
		//17/11/2011
		//For AP Payments  That Are Charges ,The Amounts Will Be Used To Close The Open Amount IN The Tax Returns.
		...
		if( docBaseType != null ) {
			...
			if( docBaseType.equalsIgnoreCase(MDocType.DOCBASETYPE_APPayment) ) {
			
				int paymentID = this.getC_Payment_ID();
				if( paymentID > 0 && !isFromcreatepayment() ){
					
					//Getting Record Of Type Charge Allocations , Not Invoice Allocation
					String whereClause = MPaymentAllocate.COLUMNNAME_C_Payment_ID+" = "+paymentID +" AND "+
					                     MPaymentAllocate.COLUMNNAME_C_Charge_ID +" IS NOT NULL";
					List <MPaymentAllocate>  paymentAllocations = new Query(getCtx(), 
							                                                MPaymentAllocate.Table_Name, 
							                                                whereClause, 
							                                                this.get_TrxName()).list();
					
					if( paymentAllocations != null && paymentAllocations.size() > 0){
						
						for( MPaymentAllocate   paymentAllocate : paymentAllocations ) {
							
							if( paymentAllocate != null ) {
								//This Method Is responsible For Getting The  Charges Related To Resultant Payment Allocation
								allocateCharge( paymentAllocate );
								
							}else {
								log.log(Level.SEVERE, "PaymentAllocate Is Null So Nothing To Do.");
							}
						}
					}else{
						log.log(Level.SEVERE, "There Are No MPaymentAllocate Records Which Are Charges For This Payment");
					}
				}
			}
		  }
...
*/
			...
			cash.processIt(DOCACTION_Complete);
		}
		// End Trifon - CashPayments
	 ...
	 * Date : 17/11/2011
	 */
	private void allocateCharge( MPaymentAllocate paymentAllocate ) {
		
		if (paymentAllocate != null) {
			
			BigDecimal paymentAllocationAmount = paymentAllocate.getAmount();
			int paymentAllocationID = paymentAllocate.getC_PaymentAllocate_ID();
			//If The Payment Allocation Amount Is Grater Than Zero Then Only We Will Go For Return Those Amounts.
			if(paymentAllocationAmount != null && paymentAllocationAmount.compareTo(Env.ZERO) == 1) {
				
				int charge = paymentAllocate.getC_Charge_ID();
				if( charge > 0 ) {
					
					String whereClauseTaxIndicator = X_WTC_TaxIndicator.COLUMNNAME_C_Charge_ID +" = "+charge;
					List<X_WTC_TaxIndicator>  taxIndicators = new Query(getCtx() ,  
							   											X_WTC_TaxIndicator.Table_Name, 
							   											whereClauseTaxIndicator, 
							   											this.get_TrxName()).list();
					if (taxIndicators !=null && taxIndicators.size() > 0) {
						
						for(X_WTC_TaxIndicator taxIndicator : taxIndicators) {
							if ( paymentAllocationAmount != null && paymentAllocationAmount.compareTo(Env.ZERO) == 1 ) {
								if(taxIndicator != null) {
									//Getting The Tax Returns With Respect to This Tax Indicators
									paymentAllocationAmount = allocateTaxReturn(taxIndicator ,paymentAllocationAmount ,paymentAllocationID );
								}else {
									log.log(Level.SEVERE,  "TaxIndicator Is Null So Nothing To Do.");
								}
							}else {
								log.log(Level.SEVERE, "Payment Allocation Amount is Distributed Among Retruns");
							}
						}
					}else{
						log.log(Level.SEVERE, "There Are No Tax Indicators With This Charge"+charge);
					}
					
				}else {
					log.log(Level.SEVERE, "Even Though We Have Got The Alloations Which Are Of Type Charges " +
							              " For This PaymentAllocate "+paymentAllocationID+" We can't Able To Get Charge ");
				}
			}
		}
	}
	 ...
	 * Date : 17/11/2011
	 */
	private BigDecimal allocateTaxReturn(X_WTC_TaxIndicator  taxIndicator , BigDecimal paymentAllocationAmount , int paymentAllocationID) {
		
		if(taxIndicator != null) {
			int taxIndicatorID =taxIndicator.getWTC_TaxIndicator_ID();
			log.log(Level.INFO, "Tax Indicator ID ="+taxIndicatorID+" AND Allocation Amount ="+paymentAllocationAmount +" AND PaymentAllocation ID="+paymentAllocationID);
			 
			String whereClause = MWTCTaxReturnForm.COLUMNNAME_WTC_TaxIndicator_ID +" = "+taxIndicatorID+ " AND "+
			   					 MWTCTaxReturnForm.COLUMNNAME_OpenAmt +"> "+0 + " AND "+
			   					 MWTCTaxReturnForm.COLUMNNAME_DocStatus+"='"+MWTCTaxReturnForm.DOCSTATUS_Completed+"'";
			List<MWTCTaxReturnForm> taxReturns = new Query(getCtx(), MWTCTaxReturnForm.Table_Name,whereClause,this.get_TrxName())
			                                               .setOrderBy(MWTCTaxReturnForm.COLUMNNAME_DateAcct + " ASC").list();
			
			if (taxReturns != null && taxReturns.size() > 0) {
				
				BigDecimal remainingAmount = Env.ZERO;
				for (MWTCTaxReturnForm taxReturn : taxReturns) {
					
					//if paymentAllocationAmount Is Zero Means Allocation Amount is Distributed Completely
					if(paymentAllocationAmount.compareTo(Env.ZERO) != 1) {
						break;
					}
					
					if (taxReturn != null) {
						BigDecimal openAmount = taxReturn.getOpenAmt();
						
						if (openAmount.compareTo(paymentAllocationAmount) == 1) {  
							//if Payment Allocation Amount is 500  And Open Amount is 600,Then Remaining Open Amount = 600-500 = 100 , And Payment Allocation Amount is Over , So ZERO
							remainingAmount = openAmount.subtract(paymentAllocationAmount);
							paymentAllocationAmount= Env.ZERO;
						}else if (openAmount.compareTo(paymentAllocationAmount) <= 0) {
							//if Payment Allocation Amount is 600  And Open Amount is 500,Then Payment Allocation Amount = 600-500 = 100 , And Remaining Open Amount is Over , So ZERO
							remainingAmount = Env.ZERO;
							paymentAllocationAmount= paymentAllocationAmount.subtract(openAmount);
						}
						
						taxReturn.setOpenAmt(remainingAmount);
						
						if (!taxReturn.save()) {
							log.log(Level.SEVERE, "Can't Save Tax Returns"+taxReturn.getWTC_TaxReturnForm_ID());
						}else {  //Record Is Saved 
							int taxRetrunID = taxReturn.getWTC_TaxReturnForm_ID();
							createMWTCTaxretrunPaymentAlllocation(remainingAmount ,paymentAllocationID , taxIndicatorID , taxRetrunID ,getCtx() , get_TrxName() );
						}
					} else {
						log.log(Level.SEVERE,"Tax Return Form is Null So  Nothing To Do ");
					}
				}
			} else {
				log.log(Level.SEVERE, "TaxReturns are  Null So Nothing To Do.");
			}
			
			log.log(Level.INFO, "Remainig Payment  Allocation Amount ="+paymentAllocationAmount);
		}
	 ...
	 * This Table Will Hold The Data Related To Open Amount of Tax Retruns That Is Filled With Allocation Amount Amount Of The Charge
	 * @param amount
	 * @param paymentAllocationID
	 * @param taxIndicatorID
	 * @param taxReturnID
	 * @return
	 * 
	 * Date : 17/11/2011
	 */
	public static boolean createMWTCTaxretrunPaymentAlllocation ( BigDecimal amount , int paymentAllocationID , int taxIndicatorID , int taxReturnID , Properties ctx , String trxName)
	{
		X_WTC_TaxComp_PayDetails   payallocForTaxReturn = new X_WTC_TaxComp_PayDetails( ctx , 0, trxName );
		payallocForTaxReturn.setC_PaymentAllocate_ID( paymentAllocationID );
		payallocForTaxReturn.setWTC_TaxIndicator_ID( taxIndicatorID );
		payallocForTaxReturn.setWTC_TaxReturnForm_ID( taxReturnID );
		payallocForTaxReturn.setAmount(amount );
		
		if(! payallocForTaxReturn.save()) {
			CLogger.getCLogger( "MPayment").log( Level.SEVERE,  "WTC_TaxreturnPaymentAllocation  Entry Creation Failed" );
			return false;
		}
		else {
			return true;
	...
	public boolean allocateIt()
	{
		...
		for (int i = 0; i < pAllocs.length; i++)
		{
			...
			if (isReceipt() )//||pa.getC_Charge_ID()>0)
