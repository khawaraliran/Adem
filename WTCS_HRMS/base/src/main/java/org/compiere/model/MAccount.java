 ...
 * 	@ChangeID	[201202181145]
 *  @ProblemStatement	
 *   <li>if we didn't get an account 
 *   <li>then system creating a new account and trying to save the record, At that time it doesn't take care about existing transaction, 
 *   <li> PO creates a new transaction and trying to commit[we have already one transaction it also modifying same table] then database locks are occurred 
 *  @Description	Introduced one more constructor with saveNew argument now we are saving the newaccount based on the saveNew argument 
 *  
 */
public class MAccount extends X_C_ValidCombination
{
	...
	public static MAccount get(Properties ctx, int AD_Client_ID, int AD_Org_ID,
		...
		//
		// 201202181145
		//	we are calling another Constructor (by default saveNew is true )
		//

		return get(ctx, AD_Client_ID, AD_Org_ID, C_AcctSchema_ID,
				Account_ID, C_SubAcct_ID, M_Product_ID, C_BPartner_ID,
				AD_OrgTrx_ID, C_LocFrom_ID, C_LocTo_ID, C_SalesRegion_ID,
				C_Project_ID, C_Campaign_ID, C_Activity_ID, User1_ID, User2_ID,
				UserElement1_ID, UserElement2_ID, true);
	...
	public static MAccount get (Properties ctx, 
		...
		int C_LocFrom_ID, int C_LocTo_ID, int C_SalesRegion_ID, 
		int C_Project_ID, int C_Campaign_ID, int C_Activity_ID,
		int User1_ID, int User2_ID, int UserElement1_ID, int UserElement2_ID, boolean saveNew)
	{
		...
		//
		//	201202181145
		//	If SaveNew is true then allowing to save the newAccount
		//	otherwise return MAccount object without saving 
		//
