 ...
 *  @version $Id: GridField.java 1906 2013-06-25 18:44:36Z vinod.kakumani $
 */
/****************************************************************************************************************************************************************
 * @author     			@Date        	@BugNo       @Description
 * Anitha.k   		[20111213]      	 1683		 If Any table Having IsSystemGenerated Column,
 * 													That Table Having name Column,It Sets 
 *                                          		In Context That Name Column As ReadOnly
 *
 * D.Yadagiri Rao	[20120106]			 1722		Window Customization purpose i modified this class (Applying Security And Roles)
 * 
 * Anitha.K         [20120229]           2059      If Field having Always Updatable Check box='Y' And Read Only Logic, First Priority Gives To read Only Logic
 *                                                 else field is Always Updatable
 *                                                 And Checks the Record is Processed Check 'm_vo.IsAlwaysUpdateable' is false condition
 * K.Vinod          [20120625]          20040   	Introduced new variable isRange and added new method isRange()  (AD_Column) 
 *****************************************************************************************************************************************************************/
public class GridField 
	implements Serializable, Evaluatee
{
// Kindly do not delete below line as it is being used for svn version maintenance
public static final String svnRevision =  "$Id: GridField.java 1906 2013-06-25 18:44:36Z vinod.kakumani $";
	/**
	 * 
	 */
...
//	  Do we have a mandatory rule
	...
	public boolean isEditable (boolean checkContext)
	{
		if (isVirtualColumn())
			return false;
	    //[20111213]
		if(m_vo.ColumnName.equalsIgnoreCase("Name"))
		{
			String isSystemGen=Env.getContext(m_vo.ctx, m_gridTab.getWindowNo(),m_gridTab.getTabNo(), "IsSystemGenerated", true, true);
			
        	if(isSystemGen != null)
	    	{
    			boolean isSystemGenerated ="Y".equalsIgnoreCase(isSystemGen);
    			if(isSystemGenerated)
    			{
    				m_vo.IsReadOnly=true;
	        	}
	        	else
	        	{
	        		m_vo.IsReadOnly=false; 
	        	}
		     }
         } 
		...
		//[20120229]
		if ( m_vo.IsAlwaysUpdateable &&  m_vo.ReadOnlyLogic.trim().isEmpty() )      //  Zoom
			return true;
		...
		{
			//
			// Check ReadOnly Logic starts With @WTC or NOT
			//
			
			if (m_vo.ReadOnlyLogic.trim().startsWith("@WTC=")) {
				Boolean readOnly = Boolean.FALSE;
				String roLogic = m_vo.ReadOnlyLogic.substring(5);
				
				// 
				// Replace The Context Variables With Values 
				//
				
				roLogic = Env.parseContext(m_vo.ctx, m_vo.WindowNo, roLogic, false, false); // replace variables
				
				if ( roLogic.trim().isEmpty() ) {
					log.log(Level.WARNING, "(" + m_vo.ColumnName
							+ ") - ReadOnly SQL variable parse failed: "
							+ m_vo.ReadOnlyLogic);
				} else {
					readOnly = DB.getSQLValueBoolean(roLogic, null);
					if (readOnly)
						return ! readOnly;
				}
				...
				boolean retValue = !Evaluator.evaluateLogic(this,
						m_vo.ReadOnlyLogic);
				log.finest(m_vo.ColumnName + " R/O(" + m_vo.ReadOnlyLogic
						+ ") => R/W-" + retValue);
				if (!retValue)
					return false;
			}
	 ...
	 */
	Boolean  isIDField = null;
	public boolean isDisplayed (boolean checkContext)
	{
		if(m_vo.AD_Table_ID ==107)
		{
			  if(m_vo.ColumnName.equalsIgnoreCase("AD_Column_ID"))
			  {	
				int columnID;
				if((Integer)this.getValue() != null)
				{
					columnID=(Integer)this.getValue();
					
			    	String sql="SELECT columnname from ad_column where ad_column_id=?";
				    String columnName1=DB.getSQLValueString(null, sql, columnID);
				    columnName1 = columnName1.substring(columnName1.lastIndexOf("_")+1);
				    
//				    String sql1="SELECT ad_reference_id from ad_column where ad_column_id=?";
//				    int columnName2=DB.getSQLValue(null, sql1, columnID);
				    
				    if(columnName1.equalsIgnoreCase("ID") ) // || columnName2 == 28) // For button
				    {
				    	 isIDField=true;
				    }
				    else
	...
	/** 
	 * range
	 * @return true if isRange Selected only
	 */
	public boolean isRange()								//20040
	{
		return m_vo.IsRange;
	}
	/**
	 ...
	 * @author Giri
	 * <BR>
	 * Added one more argument to getSql Method 
	 * 
	 * 
	 * 	Create Fields.
		 ...
		 int AD_Tab_ID)
	{
		...
		//
		// Here we passing false 
		// 
		//TODO this should Enhance, IN Find Window(Lookup) Advance Tab columns Drop down we  would get All fields   
		//
		
		String sql = GridFieldVO.getSQL(ctx, false);
	...
	*/
	 ...
	 * 
	 * <BR>Setting boolean value to IsReadOnly Constant
	 * <BR>This method We can use in Callout By mField
	 * <BR>We can apply readOnly logic for a field By using this method
	 * 
	 */
	public void setReadonly( boolean isReadonly){
		m_vo.IsReadOnly	=	isReadonly;
	}
}   //  MField
