 ...
 *
 * 	@author 	Jorg Janke
 * 	@version 	$Id: LayoutEngine.java 1009 2012-02-09 09:16:13Z suman $
 * 
 ...
 ******************************************************************************************************************************************************
 * 1647     Arunkumar     [20120112:12:44]       Modified : createStandardHeaderFooter() 
 *                                                          Which is Responsible For Printing The Copy Right Text .
 * 1647     Arunkumar     [2012016:10:00]		 Modified : layout() Which Gives A Gap After Immage placement  in the report print
 * 1647     Arunkumar     [2012016:10:05]		 Modified : layoutForm()  : If Header Footer Is null it will give null pointer exception so modified this code
 * 1647     Arunkumar     [2012016:10:30]        Added    : createTableElement()  method to Create Table Element Based on Our Requirement
 *                                                              This method is used to create Table Element According to T_Reports Table Entry
 * 1647     Arunkumar     [2012016:10:45]        Added    : setWidthAndHeightOfPage(TableElement)  : This method is used to adjust the size of the page.    
 * 1647	    Phani.Gutha   [20120124:02:52]	     Mofified : using table id from the I_T_ReportClass instead of EagleConstant variable , null check in setWidthAndHeightOfPage()
 * 1647     Arunkumar     [20120124:4:30PM]      Modified : createStandardHeaderFooter()  Set the Y Position of the footer , so that footer content Will not Over ride
 * 1647     Arunkumar     [20120124:4:45PM]      Modified : layout() : Increased Content Y Position to Support Big Immages.
 * 	                                                                                           
 */
public class LayoutEngine implements Pageable, Printable, Doc
{
// Kindly do not delete below line as it is being used for svn version maintenance
public static final String svnRevision =  "$Id: LayoutEngine.java 1009 2012-02-09 09:16:13Z suman $";
	/**
	 *	Detail Constructor
	...
	private void layout()
	{
		...
		else
		{
			...
			if (element != null)
			{
				//[2012016:10:00], [20120124:4:45PM]
				//Which Gives A Gap After Immage placement
				m_position[AREA_CONTENT].y += 25;
	...
	//	element.layout (m_footer.width, 0, true, MPrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
		...
		element.layout (ft.x, 0, true, MPrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
	
		
		element.setLocation(ft);
		m_headerFooter.addElement(element);
		//
		ft.x += element.getWidth();
		
		element = new StringElement(Adempiere.COPYRIGHT, font, color, null, true);
		element.setLocation(ft);
		m_headerFooter.addElement(element);
		...
		element = new StringElement(" "+client.getName() + " - " + org.getName(), font, color, null, true);
		...
		for (int row = 0; row < m_data.getRowCount(); row++)
		{
			...
			//		+ " w=" + lastWidth[m_area] + ",h=" + lastHeight[m_area] + " " + item);
				...
				if (m_area == AREA_CONTENT)
					...
					//[2012016:10:05]
					//If Header Footer Is null it will give null pointer exception so modified this code
					if(m_headerFooter != null) {
					  m_headerFooter.addElement (element);
					}else {
						log.log(Level.SEVERE, "Header Footer Object is Not Created So Creating New Object");
						m_headerFooter = new HeaderFooter(m_printCtx);
						m_headerFooter.addElement (element);
					}
				}
	...
	//	System.out.println("Rows=" + rows);
		...
		TableElement table =createTableElement( columnHeader,
	 ...
	 * @return
	 */
	private TableElement createTableElement(ValueNamePair[] columnHeader,
											int[] columnMaxWidth, int[] columnMaxHeight,
											String[] columnJustification, boolean[] fixedWidth,
											ArrayList<Integer> functionRows, boolean multiLineHeader,
											Object[][] data, KeyNamePair[] pk, String pkColumnName,
											int pageNoStart, Rectangle firstPage, Rectangle nextPages,
											int repeatedColumns, HashMap<Integer, Integer> additionalLines,
											HashMap<Point, Font> rowColFont, HashMap<Point, Color> rowColColor,
											HashMap<Point, Color> rowColBackground, MPrintTableFormat tf,
											ArrayList<Integer> pageBreak, boolean[] colSuppressRepeats) {
		
		//
		TableElement table = null;
		//[20120124:02:52]
		if( m_format.getAD_Table_ID() == I_T_Report.Table_ID ) {
			
			table = new TableElement(columnHeader,
									 columnMaxWidth, columnMaxHeight, columnJustification,
									 fixedWidth, functionRows, multiLineHeader,
									 data, pk, pkColumnName,
									 pageNoStart, firstPage, nextPages, repeatedColumns, additionalLines,
									 rowColFont, rowColColor, rowColBackground,
									 tf, pageBreak, colSuppressRepeats, I_T_Report.Table_ID );
			
			table.layout(0,0,false, MPrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
			if (m_tableElement == null)
				m_tableElement = table;
			
			//This method is used to adjust the size of the page.
			setWidthAndHeightOfPage(table);
	    	...
	    	table = new TableElement(columnHeader,
	    			...
	    			pageNoStart, firstPage, nextPages, repeatedColumns, additionalLines,
	    			rowColFont, rowColColor, rowColBackground,
	    			tf, pageBreak, colSuppressRepeats,0);
	 ...
	 * @param table
	 */
	private void setWidthAndHeightOfPage(TableElement table) {
		
		//[20120124:02:52]
		if( table == null ){
			log.warning( " TableElement not found " );
			return ;
		}
		
		int widthPage = table.finalWidth ;
		m_content.width = widthPage;
	    
		int y = (int)m_paper.getImageableY (true);
		int h = (int)m_paper.getImageableHeight (true);
		
		Double paperHeight = m_paper.getHeight();
		
		int height = m_headerHeight;
		y += height;
		height = h-m_headerHeight-m_footerHeight;
		paperHeight = new Double(paperHeight.intValue()+height+y+h) ;
		m_paper.setSize(widthPage, paperHeight);
		
		int x = (int)m_paper.getImageableX (true);
		m_header.setBounds(x, y, widthPage, height);
		
		y += height;
		height = m_footerHeight;
		m_footer.setBounds(x, y, widthPage, height);
