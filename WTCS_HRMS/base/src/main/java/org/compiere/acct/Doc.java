 ...
 *  @version  $Id: Doc.java 1171 2012-03-21 10:00:09Z anitha $
 */
public abstract class Doc
{
// Kindly do not delete below line as it is being used for svn version maintenance
public static final String svnRevision =  "$Id: Doc.java 1171 2012-03-21 10:00:09Z anitha $";
	...
	/** Tax Return */
	public static final String DOCTYPE_TaxReturn = X_C_DocType.DOCBASETYPE_TaxReturn;
	...
	public int getValidCombination_ID (int AcctType, MAcctSchema as)
	{
		...
		try
		{ 
			//[20120320] added transaction due to transaction problem in Junit
			//BugNo :2435
	...
	public static void createProductPo( Doc doc , DocLine[] p_lines ){
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		
		try {
			String existsSQl = " SELECT EXISTS(  " +" SELECT * FROM M_Product_PO WHERE  AD_Client_ID = ? AND AD_Org_ID = ? AND  M_Product_Id = ? AND C_BPartner_ID = ? "+ " AND C_Currency_ID = ? AND C_UOM_ID = ?   ) ";
			 pstmt = DB.prepareStatement( existsSQl );

			for( DocLine line : p_lines ){

				rs = Doc_Order.ProductPOExists(doc, line, pstmt);
				if( rs.next() ){
					...
					if( !exists ){
						...
						MProductPO ppo = new MProductPO(doc.getCtx(), 0, doc.getTrxName() );
						ppo.setAD_Org_ID( doc.getAD_Org_ID() );
						ppo.setC_BPartner_ID( doc.getC_BPartner_ID() );
						ppo.setC_Currency_ID( doc.getC_Currency_ID() );
						ppo.setC_UOM_ID( line.getC_UOM_ID() );
						ppo.setM_Product_ID( line.getM_Product_ID() );
						ppo.setVendorProductNo( String.valueOf(  line.getM_Product_ID() ) + doc.m_C_BPartner_ID + doc.getDocumentNo() ); 
						
						if ( ppo.save() ){
							
							s_log.severe( " MProductPO Save failed " );
						}
					}
				}
	...
	public static ResultSet ProductPOExists( Doc doc , DocLine line , PreparedStatement pstmt ) throws SQLException{
		


		pstmt.setInt( 1 , doc.getAD_Client_ID() );
		pstmt.setInt( 2 , doc.getAD_Org_ID() );
		pstmt.setInt( 3 , line.getM_Product_ID() );
		pstmt.setInt( 4 , doc.getC_BPartner_ID() );
		pstmt.setInt( 5 , doc.getC_Currency_ID() );
		pstmt.setInt( 6 , line.getC_UOM_ID() );
