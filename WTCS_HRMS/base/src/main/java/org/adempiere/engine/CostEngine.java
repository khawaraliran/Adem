 ...
 *@ChangeID        20111226912
 *@Description 	   
 */
public class CostEngine
{
// Kindly do not delete below line as it is being used for svn version maintenance
public static final String svnRevision =  "$Id: CostEngine.java 1009 2012-02-09 09:16:13Z suman $";
	/**	Logger							*/
	...
	public void createCostDetail(MAcctSchema as, MTransaction mtrx ,IDocumentLine model , MCostType ct, MCostElement ce)
	{
		...
		{	
			...
			{	
				...
				{// get the original indentline and its reference Transaction, through the reference transaction get its cost details to get the cost.
					MInventoryLine inventoryLine = (MInventoryLine) model;
					MTransaction refTrx =  inventoryLine.getRefTransaction(mtrx.getM_AttributeSetInstance_ID());
					if( refTrx != null ){
						MCostDetail mcsd = MCostDetail.getByTransaction( inventoryLine.getRefTransaction(mtrx.getM_AttributeSetInstance_ID()), as.getC_AcctSchema_ID(), ct.get_ID(), ce.get_ID() );
						costThisLevel = mcsd.getCurrentCostPrice();
					} else if( MCostElement.COSTELEMENTTYPE_Material.equals(ce.getCostElementType()) ){// ref transaction doesnot exits then get the current cost price in the inventory line
						costThisLevel = inventoryLine.getCurrentCostPrice(); 
					}
				}
				//get the costdetails which is create for the Movementfrom to get the cost price of the transaction
				if(model instanceof MMovementLine ) 
				{
					MTransaction trx = MTransaction.getByDocumentLine(model, MTransaction.MOVEMENTTYPE_MovementFrom);
					MCostDetail mcsd = MCostDetail.getByTransaction( trx, as.getC_AcctSchema_ID(), ct.get_ID(), ce.get_ID() );
					costThisLevel = mcsd.getCurrentCostPrice();
				}
			} 
			else if((model instanceof MInOutLine && MTransaction.MOVEMENTTYPE_CustomerReturns.equals(mtrx.getMovementType()))){
				// get the transaction of the Customer shipment to get its cost in the cost detail of the reference transaction
				MInOutLine line = (MInOutLine)model;
				MCostDetail mcsd = MCostDetail.getByTransaction( line.getRefTransaction(mtrx.getM_AttributeSetInstance_ID()), as.getC_AcctSchema_ID(), ct.get_ID(), ce.get_ID() );
				costThisLevel = mcsd.getCurrentCostPrice();
				
			}
			else if (MCostElement.COSTELEMENTTYPE_Material.equals(ce.getCostElementType()) )   
			{	// cost element is material get the actual price of the material 
				if( model instanceof MLandedCostAllocation){
					//20111226912
					MInvoiceLine invLine = MInvoiceLine.getOfInOutLine( (MInOutLine) mtrx.getM_InOutLine() );
					costThisLevel = invLine.getPriceActual();
					
					if( costThisLevel.signum() == 0 ){
						costThisLevel = ( (MInOutLine)mtrx.getM_InOutLine() ).getPriceActual();
					}
				}else{
